...
var DATA_SOURCE = "MapBiomas Alerta v2";
...
var onCronTrigger = (runtime2) => {
  runtime2.log("PATROL ANALISE - BUSCA CAR ON-CHAIN");
  ...
  const reportData = runtime2.runInNodeMode((nodeRuntime) => {
    ...
    const listarData = encodeFunctionData2({
      abi: ABI,
    ...
    runtime2.log(`CAR vindo do contrato: ${codigoCAR}`);
    if (!codigoCAR) {
      return "CAR_VAZIO";
    }
    const cidTerritorial = lerStringOnchain(httpClient, nodeRuntime, address, "cidTerritorial", [codigoCAR], 2);
    if (!cidTerritorial) {
      runtime2.log("CAR ainda nao tem cidTerritorial - pula analise");
      return "SEM_TERRITORIAL";
    }
    runtime2.log(`CID territorial on-chain: ${cidTerritorial}`);
    ...
    if (propJson?.errors) {
      runtime2.log(`MapBiomas errors: ${JSON.stringify(propJson.errors)}`);
    }
    const propData = propJson?.data?.ruralProperty;
    if (!propData?.propertyCode) {
      runtime2.log("CAR nao encontrado no MapBiomas");
      return "CAR_NAO_ENCONTRADO";
    }
    runtime2.log(`CAR encontrado no MapBiomas: ${propData.propertyCode}`);
    runtime2.log(`Area: ${propData.areaHa}ha | Estado: ${propData.stateAcronym}`);
    const alerts = Array.isArray(propData.alerts) ? propData.alerts : [];
    const totalAlerts = alerts.length;
    const areaHaAlertas = alerts.reduce((total, alerta) => total + Number(alerta?.areaHa ?? 0), 0);
    const deforestationStatus = totalAlerts > 0 ? 1 : 0;
    const embargoStatus = 0;
    const alertHectares = Math.round(areaHaAlertas * 100);
    runtime2.log(`Alertas: ${totalAlerts} | ${areaHaAlertas}ha`);
    runtime2.log(`deforestationStatus=${deforestationStatus}`);
    runtime2.log("embargoStatus=0 nesta versao sem IBAMA para caber no limite de 5 calls");
    ...
    const sourceHash = keccak2562(toBytes3(JSON.stringify(analiseContent)));
    ...
    const cidAnaliseNovo = decodeBody(pinataAnalise.body)?.IpfsHash;
    if (!cidAnaliseNovo) {
      runtime2.log("ERRO IPFS analise");
      return "ERRO_IPFS_ANALISE";
    }
    runtime2.log(`CID Analise: ${cidAnaliseNovo}`);
    runtime2.log("Montando report action=2");
    return encodeAbiParameters2(REPORT_ANALISE_PARAMS, [
      2,
      codigoCAR,
      {
        deforestationStatus,
  ...
  runtime2.log(`Report analise hex: ${reportData}`);
