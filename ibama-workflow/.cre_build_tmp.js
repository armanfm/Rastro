...
var IBAMA_URL = "https://pamgia.ibama.gov.br/server/rest/services/01_Publicacoes_Bases/adm_embargos_ibama_a/FeatureServer/0/query";
var PINATA_URL = "https://api.pinata.cloud/pinning/pinJSONToIPFS";
var DATA_SOURCE = "IBAMA PAMGIA via CID Territorial";
...
function lerStringOnchain(httpClient, nodeRuntime, address, functionName, args, id) {
  try {
...
function consultarIBAMA(httpClient, nodeRuntime, boundingBox) {
  try {
    ...
    nodeRuntime.log(`IBAMA boundingBox [${lonMin.toFixed(4)},${latMin.toFixed(4)}] -> [${lonMax.toFixed(4)},${latMax.toFixed(4)}]`);
    const resp = httpClient.sendRequest(nodeRuntime, {
...
var onCronTrigger = (runtime2) => {
  runtime2.log("PATROL ANALISE - IBAMA ONLY");
  ...
  const reportData = runtime2.runInNodeMode((nodeRuntime) => {
    ...
    const cidTerritorial = lerStringOnchain(httpClient, nodeRuntime, address, "cidTerritorial", [codigoCAR], 2);
    if (!cidTerritorial) {
      runtime2.log("CAR sem cidTerritorial - pula");
    ...
    const laudoTerritorial = decodeBody(ipfsResp.body);
    const boundingBox = laudoTerritorial?.boundingBox ?? [];
    if (!Array.isArray(boundingBox) || boundingBox.length !== 4) {
      runtime2.log("CID territorial sem boundingBox valida");
      return "SEM_BOUNDING_BOX";
    }
    runtime2.log(`BoundingBox: ${JSON.stringify(boundingBox)}`);
    const embargos = consultarIBAMA(httpClient, nodeRuntime, boundingBox);
    const embargoAtivo = embargos.filter((e) => e.situacao?.toUpperCase().includes("ATIVO")).length;
    const areaEmbargada = embargos.reduce((total, e) => total + Number(e.areaHa ?? 0), 0);
    const embargoStatus = embargoAtivo > 0 ? 1 : 0;
    const deforestationStatus = 0;
    const alertHectares = 0;
    runtime2.log(`IBAMA embargos: ${embargos.length}`);
    runtime2.log(`IBAMA embargos ativos: ${embargoAtivo}`);
    runtime2.log(`IBAMA area embargada: ${areaEmbargada}ha`);
    ...
    const sourceHash = keccak256(toBytes(JSON.stringify(analiseContent)));
    ...
    const cidAnalise = decodeBody(pinataResp.body)?.IpfsHash;
    if (!cidAnalise) {
      runtime2.log("ERRO IPFS analise IBAMA");
      return "ERRO_IPFS_ANALISE";
    }
    runtime2.log(`CID Analise IBAMA: ${cidAnalise}`);
    runtime2.log("Montando report action=2");
  ...
  runtime2.log(`Report IBAMA hex: ${reportData}`);
